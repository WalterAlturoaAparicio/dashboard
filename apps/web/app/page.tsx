...
import { useConnectionStatus, API_BASE } from "@/hooks/use-connection-status"
    ...
    async (date?: Date) => {
      ...
      try {
        ...
        const [conteoRes, drawsRes] = await Promise.all([
          axios.get(`${API_BASE}/conteo?fecha=${fechaParam}&tipo=baloto`),
          axios.get(`${API_BASE}/recent-draws?tipo=baloto&cantidad=5`),
        ])
  ...
  const handleGenerate = async (cantidad: number) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/generar?tipo=baloto&fecha=${fechaParam}&tickets=${cantidad}`
      )
